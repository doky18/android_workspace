...
public class ListActivity extends AppCompatActivity {
    ...
    ListView listView;
    List<String> list = new ArrayList<String>(); //MVC 중 데이터 즉 Model
    ArrayAdapter<String> adapter; //MVC 중 컨트롤러이다.
    NoticeAdapter noticeAdapter; //복합된 아이템을 보여주기 위한 재정의된 어댑터
    Handler handler;        //개발자가 정의한 스레드가 메인 스레드로 작업하고 싶은 업무가 있을 경우, 핸들러에게 부탁하면 된다
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //리스트뷰는 보여지는 부분(view)와 데이터를 분리시켜서 구현한다.
        ...
        //R.java 의 용도: 프로젝트 환경의 구조중에서 res로 표현되는 디렉토리를 R.java로 관리가 된다
        //따라서 개발자가 이미지, xml, style 파일 등등을 자원으로 등록하면 시스템을 실시간으로 R.java로 상수로 등록을 한다
        //ex) text.xml 을 레이아웃 파일용으로 추가할 경우, res/layout/test.xml
        //R 과 android.R의 차이점
        //R : 현재 나의 프로젝트의 res (나만 사용가능)
        //android.R : 시스템차원에서 지원하는 res (나말고 다른 프로젝트도 사용가능)
        adapter = new ArrayAdapter<String>(this, android.R.layout.simple_list_item_1, list);
        noticeAdapter = new NoticeAdapter(this);            //갈아탈 수 있음

        //javaSE 처럼 new JTable(model)과 같이 뷰와 컨트롤러를 연결해야 한다
        listView.setAdapter(noticeAdapter);

        //글쓰기 버튼 이벤트 연결
        Button bt_regist = findViewById(R.id.bt_regist);
        bt_regist.setOnClickListener((v) -> {
            //안드로이드의 Activity는 시스템이 관리하기 때문에 개발자가 new할 수 없다
            //시스템에 의해서 관리는 Activity는 생명주기가 존재하고, 그 생명주기에 맞게
            //개발자는 적절한 코드를 작성만 하면 된다... 웹에서의 서블릿의 서버가 관리하는 것처럼
            //개발자는 안드로이드 시스템에 무엇을 원하는지 의도를 보여야하는데
            Intent intent = new Intent(this, RegistActivity.class);        //(어디서, 어디로) 갈 의도를 가진,,
    ...
    public void convertJsonToList(JSONArray jsonArray) {
        ...
        try {
            ...
            //핸들러에게 부탁하기 ---> 옮겨감 noticeAdapter.notifyDataSetChanged();
    ...
    public void requestList() {
        ...
        try {
            URL url = new URL("http://172.30.1.54:7777/rest/notice/list");
